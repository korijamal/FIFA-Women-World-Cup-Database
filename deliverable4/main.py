...
default_file_path_mac = initial_path+"cs425_fifa_fa24_db_setup.sql"
# Placeholder for default file path
# default_file_path = "deliverable4/cs425_fifa_fa24_db_setup.sql"
default_file_path = initial_path+"/cs425_fifa_fa24_db_setup.sql"
# default_file_path = "deliverable4/cs425_fifa_fa24_db_setup.sql"
cursor = None
connection = None
...
def show_frame(frame):
    ...
    if frame == frames['connecting']:
        ...
        start_connecting_animation(connecting_label, count = 0)
...
def save_defaults():
    ...
    if host and user:
        ...
        messagebox.showinfo("Success","Default values updated successfully.")
        show_frame(frames["create_db"])
    else:
...
def start_gui():
    ...
    tk.Label(frames["defaults"], text="Current Defaults:", font=("Arial", 12)).pack(pady=5)
    ...
    tk.Button(frames["defaults"], text="Use Current Defaults", command=use_defaults).pack(pady=5)
    tk.Button(frames["defaults"], text="Save New Defaults and Proceed", command=save_defaults).pack(pady=10)
    ...
    tk.Button(frames["main_menu"], text="Back to Defaults", command=lambda: show_frame(frames["defaults"])).pack(pady=5)
...
def display_df_update(frame, df, table_name):
    ...
    # Persistent dictionary to hold BooleanVars for column checkboxes
    ...
    def open_update_frame(row_index, columns):
        ...
        def submit_updates():
            ...
            if new_values:
                ...
                # Update the database here as well
                # Example:
                # update_database_row(row_index, new_values)
...
def display_df_delete(frame, df, table_name):
    ...
    def delete_selected():
        ...
        # Update the database here as well if necessary
        # Example:
        # delete_database_row(selected_row_index)
...
def custom_query():
    ...
    def run_query():
        ...
        if query:
            try:
                # Assuming there's a function to execute custom SQL queries
                try:
                    result_df = ms.query_to_df(query, cursor, connection, commit = False)
                except:
                ...
                messagebox.showerror("Query Error", f"Unable to execute query. Error: {e}")
        else:
